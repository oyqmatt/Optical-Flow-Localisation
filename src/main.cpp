...
int main(int argc, char **argv) {
  ...
  // Camera Parameters
  // Mat cam_mtx = (Mat_<float>(3,3) << 8787.6 / 5472 * 1280, -1.2473, 2794.8 /
  // 5472 * 1280,
  ...
  // Mat dist_coeff = (Mat_<float>(5,1) << 0.5566, -16.7126, -0.0042, 0.0046,
  // 221.9779);
  ...
  // cv::initUndistortRectifyMap(cam_mtx,distCoeff,R,
  // new_cam_mtx,distorted.size(),CV_32FC1,map1,map2);
  // cv::remap(distorted,old_frame,
  // map1,map2,cv::InterpolationFlags::INTER_LINEAR);
...
#ifdef VID_OUT
  VideoWriter vid_out("output.avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
                      30, old_frame.size());
#endif
  ...
  while (true) {
    ...
    cout << "Current Location: X = " << x_global << ", Y = " << y_global
         << ", Angle = " << angle << endl;

    Mat img;
    add(frame, mask, img);
    cv::putText(img,
                "Current Location: X = " + to_string(x_global) + ", Y = " +
                    to_string(y_global) + ", Angle = " + to_string(angle),
                cv::Point(5, 20),  // Coordinates (Bottom-left corner of the
                ...
                cv::LINE_AA);
    circle(img,
           Point(-x_global + frame.size().width / 2,
                 y_global + frame.size().height / 2),
           5, CV_RGB(0, 0, 0), -1);
    imshow("Frame", img);
